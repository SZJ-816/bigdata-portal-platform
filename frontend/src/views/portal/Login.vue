  ...
  <div class="login-page">
    <div class="login-card">
      <div class="login-header">
        <h2>{{ isLogin ? '登录' : '注册' }}</h2>
        <p class="login-subtitle">{{ isLogin ? '登录科技资讯平台' : '创建新账号' }}</p>
      </div>
...
async function handleSendCode() {
  ...
  try {
    ...
    if (res.data.success) {
      ...
      errorMsg.value = res.data.error || '发送失败'
    }
  } catch (err) {
    errorMsg.value = err.response?.data?.error || '验证码发送失败，请稍后重试'
  } finally {
...
async function handleSubmit() {
  ...
  try {
    ...
    errorMsg.value = isLogin.value ? '用户名或密码错误' : (err.response?.data?.error || '注册失败，请重试')
  } finally {
    loading.value = false
  }
}
