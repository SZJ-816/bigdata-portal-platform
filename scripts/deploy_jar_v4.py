#!/usr/bin/env python3
"""Rebuild JAR - include resources in BOOT-INF/classes"""
...
def main():
    ...
    print("\n=== Step 1: Copy resources to classes dir ===")
    # Copy resources (application.yml etc.) into classes_dir
    if os.path.exists(resources_dir):
        copy_tree(resources_dir, classes_dir)
        print("Copied resources to classes dir")
    else:
        print(f"WARNING: {resources_dir} not found")

    # Check what files are in classes_dir now
    print("\n=== Step 2: Rebuild JAR ===")
    rebuild_dir = "/tmp/jar_rebuild4"
    ...
    print("Replaced BOOT-INF/classes (with resources)")
    ...
    # Package with jar cfm0 (store mode)
    ...
    os.chdir(base)
    out, rc = run("docker compose build backend 2>&1 | tail -5")
    print(out)

    print("\n=== Restart backend ===")
    out, rc = run("docker compose up -d backend 2>&1 | tail -5")
    print(out)
